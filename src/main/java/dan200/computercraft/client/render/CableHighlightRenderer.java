...
public final class CableHighlightRenderer
{
     ...
     */
    public static boolean drawHighlight( MatrixStack stack, VertexConsumer buffer, Entity entity, double d, double e, double f, BlockPos pos, BlockState state )
    {
        HitResult hitResult = MinecraftClient.getInstance().crosshairTarget;
        Camera info = MinecraftClient.getInstance().gameRenderer.getCamera();

        // We only care about instances with both cable and modem.
        if( state.getBlock() != Registry.ModBlocks.CABLE || state.get( BlockCable.MODEM ).getFacing() == null || !state.get( BlockCable.CABLE ) )
        {
            return false;
        }
